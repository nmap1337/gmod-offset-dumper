...
// source sdk structures (standard layout)
class RecvTable;
class RecvProp;
...
void DumpRecvTable(RecvTable* table, int offset, std::ofstream& out, std::string parent = "", std::map<std::string, std::vector<std::string>>* tableMap = nullptr) {
    ...
    for (int i = 0; i < table->m_nProps; ++i) {
        ...
        if (strcmp(prop->m_pVarName, "baseclass") == 0)
            propName = parent; // don't add "baseclass" to the name
        else
...
void DumpNetvars() {
    ...
    // 3. GetAllClasses (try vtable index 8, fallback to 10)
...
// =====================
// Entity Iteration Helper (stub)
// =====================
/*
typedef void* (*GetClientEntityFn)(void*, int);
typedef int (*GetHighestEntityIndexFn)(void*);

void ForEachEntity(std::function<void(void*)> fn) {
    void* entityList = GetInterface("client.dll", "VClientEntityList003");
    if (!entityList) return;

    auto vtable = *(void***)(entityList);
    auto getEntity = (GetClientEntityFn)vtable[3];
    auto getMaxIdx = (GetHighestEntityIndexFn)vtable[6];

    int maxIdx = getMaxIdx(entityList);
    for (int i = 0; i <= maxIdx; ++i) {
        void* ent = getEntity(entityList, i);
        if (ent) fn(ent);
    }
}
*/
...
// =====================
// use LOG_SDK for colored/time-stamped logging.
...
// =====================
// Example Usage in DumpNetvars
// =====================
// after dumping netvars, call InitializeNetvarMap(pClass);
//
// int offset = GetNetvarOffset("DT_BasePlayer", "m_iHealth");
// LOG_SDK((std::string("m_iHealth offset: 0x") + std::stringstream() << std::hex << offset).c_str()); 